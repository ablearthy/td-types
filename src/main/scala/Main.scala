...
object Utils {
  def lcIdent[_: P]: P[String] =
    P(CharIn("a-z").! ~~ CharsWhile(a => a.isLetter || a.isDigit || a == '_', 0).!).map {
...
case class BracketArg(ident: Vector[Option[String]], exclMark: Boolean, typeTerm: Term) extends Args
...
object ArgsParser {
  ...
  private def condArg[_: P]: P[Args] =
    P(Utils.varIdentOpt ~ ":" ~ conditionalDef.? ~ Utils.exclMark ~ Term.apply).map {
      case (opt, maybeDef, exclMark, term) =>
        CondArg(
          ident = opt.internal,
          cond = maybeDef,
          exclMark = exclMark,
  ...
  private def arrayArg[_: P]: P[Args] = P(
    ...
    ArrayArg(ident = maybeOpt.flatMap(_.internal), multiplicity = maybeInt, args = value.toVector)
  }

  private def bracketArg[_: P]: P[Args] = P(
    "(" ~/ Utils.varIdentOpt.rep ~ ":" ~ Utils.exclMark ~ Term.apply ~ ")"
  ).map { case (idents, exclMark, term) =>
    BracketArg(ident = idents.map(_.internal).toVector, exclMark = exclMark, typeTerm = term)
  }
...
case class AngleBracketTerm(bare: Boolean, typeIdent: TypeIdent, rest: Term.Expr) extends Term

object Term {
  type Expr = Vector[Term]
  def apply[_: P]: P[Term] = bracketTerm | angleBracketTerm | simpleTerm

  private def bracketTerm[_: P]: P[BracketTerm] = P("%".!.? ~~ "(" ~ Term.apply.rep(1) ~ ")").map {
    case (bare, terms) => BracketTerm(bare = bare.isDefined, t = terms.toVector)
  }

  private def simpleTerm[_: P]: P[SimpleTerm] = P("%".!.? ~~ TypeIdentParser.parse).map {
    case (bare, x) =>
      SimpleTerm(bare = bare.isDefined, s = x)
  }

  private def angleBracketTerm[_: P]: P[AngleBracketTerm] = P(
    "%".!.? ~~ TypeIdentParser.parse ~ "<" ~/ apply.rep(min = 1, sep = ","./) ~/ ">"
  ).map { case (bare, typeIdent, expressions) =>
    AngleBracketTerm(bare = bare.isDefined, typeIdent = typeIdent, rest = expressions.toVector)
  }
}

case class ResultType(ident: IdentWithNs, hasAngleBrackets: Boolean, terms: Vector[Term])

case class Schema(types: Vector[Definition], functions: Vector[Definition])

object ResultType {

  def apply[_: P]: P[ResultType] = P(Utils.ucIdentNs ~ (parseBrackets | parseNoBrackets)).map {
  ...
  private def parseNoBrackets[_: P] = Term.apply.rep.map(terms => (false, terms.toVector))
}
...
object MyParser {
  ...
  private def combinatorId[_: P] =
    P("#" ~~ CharIn("0-9a-f").repX(exactly = 8).!).map(Transform.hexToInt)
  ...
  private def optArgs[_: P] =
    P("{" ~/ Utils.varIdent.rep(1) ~/ ":" ~ Utils.exclMark ~/ Term.apply.rep(1) ~/ "}").map {
      case (idents, maybeExcl, terms) =>
        OptArgs(
  ...
  private def funcs[_: P] =
    P("---" ~ "functions" ~ "---" ~ definition.rep).map(defs => (true, defs.toVector))
  private def types[_: P] =
    P("---" ~ "types" ~ "---" ~ definition.rep).map(defs => (false, defs.toVector))

  def schema[_: P]: P[Schema] = P(definition.rep ~ (funcs | types).rep).map { case (fst, rest) =>
